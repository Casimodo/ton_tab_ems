...
function getMecanoPlayers(callback)
    local players = GetPlayers()
    local playerData = {}
    local job = {}
    local list_conn_mecano = {}
    ...
    for _, playerId in ipairs(players) do
        ...
        if (infos ~= nil) then
            ...
            if (infos) then
                job = infos.job
                if (job.label == 'Mécanicien') then
                    table.insert(list_conn_mecano, identifier)
                end
            end
        end
    ...
    function(user)
        for i = 1, #user do
            ...
            table.insert(playerData, {
                ...
                isConnect = table.search(list_conn_mecano, row.identifier)
            })
        end
        callback(playerData)
    end)
